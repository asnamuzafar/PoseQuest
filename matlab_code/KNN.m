...
function [meanMpjse, meanPck] = knn_search_and_evaluate(K, distance_metrics, first_iteration)
    ...
    for metric_idx = 1:length(distance_metrics)
        ...
        start_time = tic;
        [indices, distances] = knnsearch(datasetPoses, queryPoses, 'K', K, 'Distance', distance_metrics{metric_idx});
        ...
        if first_iteration
            writecell(header, 'results.csv', 'WriteMode', 'overwrite');
        end

        % Append data to the CSV file
        writecell(data, 'results.csv', 'WriteMode', 'append');
